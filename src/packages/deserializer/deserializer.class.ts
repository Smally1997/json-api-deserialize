...
class Deserializer {
  ...
  private mapResources(
    ...
    for (const resourceRelationshipKey in resource.relationships) {
      const resourceIdentifiers: IResourceIdentifier[] = asArray(
        resource.relationships[resourceRelationshipKey].data
      );
      ...
      for (const resourceIdentifier of resourceIdentifiers) {
        if (
          ...
          const includedResourceObject = this.response.included.find(
            included => {
              return (
                included.id === resourceIdentifier.id &&
          ...
          if (includedResourceObject) {
            if (Array.isArray(this.response.data)) {
              this.setNestedResourceOnIdentifierObject(
                nestedObjectPath
                  ? nestedObjectPath +
                      `.relationships[${resourceRelationshipKey}].data`
                  : null ||
  ...
  private setNestedResourceOnIdentifierObject(
    ...
    const resourceIdObject: IResourceIdentifier = get(
      this.response,
      identifierObjectPath
    );
    let fullPath: string = identifierObjectPath;

    if (resourceIdObject) {
      if (Array.isArray(resourceIdObject)) {
        const resourceIdObjectIndex: number = resourceIdObject.findIndex(
          identifierObject => {
            return (
              identifierObject.id === includedResourceObject.id &&
